...
//			System.out.println("==> " + randomAttack.getClass().getSimpleName() + ": " + randomAttack.getName());
	...
	/*public void play() throws NotEnoughKiException{
		attacker=foe;
		Fighter f1=(Fighter)foe;
		Fighter f2=(Fighter)me;
		if(f1.getKi()>=3){
			for(int i=0;i<getAssignedAttacks().size();i++){
				if(getAssignedAttacks().get(i) instanceof UltimateAttack){
					
						attack(getAssignedAttacks().get(i));
						break;
					
				}
			}	
		}
		else if(f1.getKi()>=1 && f1.getKi()<3){
			for(int i=0;i<getAssignedAttacks().size();i++){
				if(getAssignedAttacks().get(i) instanceof SuperAttack){
		...
		else if(f1.getStamina()<=1 && f1.getKi()==0){
			
				attack(getAssignedAttacks().get(0));
		...
		else if(f2.getKi()>=3 && f1.getStamina()>=3){
			block();
		}
