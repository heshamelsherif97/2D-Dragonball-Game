...
public class assignultimateattack extends JFrame implements MouseListener {
	Game g;
	JLabel background;
	JPanel k;
	JPanel k2;
	...
	ArrayList<JRadioButton> pp;
	ArrayList<JRadioButton> ii;
	...
	ArrayList<UltimateAttack> s;
	ArrayList<UltimateAttack> y;

	public assignultimateattack(Game g){
		this.g=g;
		...
		s=g.getPlayer().getActiveFighter().getUltimateAttacks();
		y=g.getPlayer().getUltimateAttacks();
		
		
		 pp=new ArrayList<JRadioButton>();
		 ii=new ArrayList<JRadioButton>();
...
int i;
	...
	int j;
				...
				UltimateAttack old=s.get(j);
				UltimateAttack newq=y.get(i);
				try {
